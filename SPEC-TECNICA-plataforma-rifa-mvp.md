...
| Derivado de CPF | Argon2id (`m=64MiB, t=3, p=1`) com salt por tenant (índice) e por campanha (leaf), nunca o mesmo salt nos dois — ADR-25 |
...
| **ADR-25** | **Derivado de CPF é Argon2id com salt por escopo** | HMAC-SHA256 com chave global (v1.x) | O espaço real de CPFs é ~10⁹ (dígitos verificadores são determinísticos) e HMAC é rápido por projeto: dump + chave no mesmo ambiente devolve a base inteira em minutos de GPU. Argon2id troca ~50ms por derivação — fora de caminho quente — por inviabilidade de enumeração em massa. Salt por tenant no índice e por campanha no leaf contém o raio de dano e impede cruzar o snapshot público com a base |
| **ADR-26** | **Operador se anonimiza, não se apaga** | `DELETE` na linha do operador | `auditoria.operador_id` e `extracao_tentativa.operador_id` são FK: apagar destruiria a atribuição de quem autorizou a entrada manual da extração — o controle do ADR-16. Um antifraude que o fraudador apaga a pedido não é controle |
